...
int Converter::RGB2GrayScale(const Mat& sourceImage, Mat& destinationImage) {
	...
	for (int row = 0; row < sourceImage.rows; row++)
	{
		for (int col = 0; col < sourceImage.cols; col++)
		{
			...
			*(destinationImage.ptr<uchar>(row, col)) =
				(A * (*sourceImage.ptr<Vec3b>(row, col))[0] + B * (*sourceImage.ptr<Vec3b>(row, col))[1] + C * (*sourceImage.ptr<Vec3b>(row, col))[2]);
		}
	}
...
int Converter::GrayScale2RGB(const Mat& sourceImage, Mat& destinationImage)
{
	...
	for (int row = 0; row < sourceImage.rows; row++)
	{
		uchar* pRow = destinationImage.ptr<uchar>(row);
		for (int col = 0; col < sourceImage.cols; col++, pRow += 3)
		{

			uchar grayScale = *(sourceImage.ptr<uchar>(row) + col);
...
int Converter::RGB2HSV(const Mat& sourceImage, Mat& destinationImage) {
	...
	for (int row = 0; row < sourceImage.rows; row++) {
		for (int col = 0; col < sourceImage.cols; col++) {
			...
			float red = (*(sourceImage.ptr<Vec3b>(row, col)))[0] / 255.0f;

			// Lấy giá trị kênh màu xanh lá tại pixel thứ (row, col)
			float green = (*(sourceImage.ptr<Vec3b>(row, col)))[1] / 255.0f;

			// Lấy giá trị kênh màu xanh dương tại pixel thứ (row, col)
			float blue = (*(sourceImage.ptr<Vec3b>(row, col)))[2] / 255.0f;
			...
			if (hue < 0) hue += 360.0f;
			(*(destinationImage.ptr<Vec3b>(row, col)))[0] = hue / 2;
			(*(destinationImage.ptr<Vec3b>(row, col)))[1] = saturation * 255;
			(*(destinationImage.ptr<Vec3b>(row, col)))[2] = value * 255;
		}
	}
...
int Converter::HSV2RGB(const Mat& sourceImage, Mat& destinationImage) {
	...
	for (int row = 0; row < sourceImage.rows; row++) {
		for (int col = 0; col < sourceImage.cols; col++) {
			float hue = (*(sourceImage.ptr<Vec3b>(row, col)))[0] * 2.0f; // 0-360
			float saturation = (*(sourceImage.ptr<Vec3b>(row, col)))[1] / 255.0f; // 0.0-1.0
			float value = (*(sourceImage.ptr<Vec3b>(row, col)))[2] / 255.0f; // 0.0-1.0
			...
			(*(destinationImage.ptr<Vec3b>(row, col)))[0] = red * 255;
			(*(destinationImage.ptr<Vec3b>(row, col)))[1] = green * 255;
			(*(destinationImage.ptr<Vec3b>(row, col)))[2] = blue * 255;
		}
	}
